<script lang="ts">
	...
	function getVixStatus(level: number | undefined): string {
		...
		if (level >= 30) return $t.mapLegend.high;
		if (level >= 20) return $t.mapLegend.elevated;
		return $t.mapLegend.low;
	}
...
<Panel
	id="commodities"
	title={getPanelName('commodities')}
	status={vixStatus}
...
>
	{#if items.length === 0 && !loading && !error}
		<div class="empty-state">{$t.noCommodityData}</div>
	{:else}
